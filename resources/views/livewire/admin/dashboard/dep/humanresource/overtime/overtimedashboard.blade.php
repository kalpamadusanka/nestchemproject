<div>
    <div class="main-panel ps ps--active-y p-2" id="main-panel">
        <div class="header d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center">
                ...
                <h4 class="me-3">Overtime management</h4>
                <nav class="nav">
        ...
        <div class="container">
            <div class="d-flex justify-content-between align-items-center ">
                <div>
                    <h4 class="fw-bold" style="margin-bottom: 0px;">Overtime</h4>
                ...
                <div class="action-buttons">
                    ...
                    <button class="btn btn-primary" wire:click="addovertimecategory">
                        <i class="bi bi-plus-circle"></i> Add Category
                    </button>
                    <button class="btn btn-primary" wire:click="addovertimerequest">
                                        ...
                                        <table id="datatable"
                                            ...
                                            <thead>
                                                <tr role="row">
                                                    ...
                                                    <th class="sorting" tabindex="0" aria-controls="datatable"
                                                        rowspan="1" colspan="1" style="width: 330px;"
                                                        aria-label="Office: activate to sort column ascending">Time Out
                                                    </th>
                                            ...
                                            <tfoot>
                                                <tr>
                                                    ...
                                                    <th rowspan="1" colspan="1">Time In</th>
                                                    <th rowspan="1" colspan="1">Time Out</th>
                                                ...
                                                <tr>
                                                    <td colspan="3" class="text-right"><strong>Total Hours Worked:</strong></td>
                                                    <td colspan="3" class="text-center">{{ $totalworkedhour ?? 0 }}h</td>
                                                </tr>
                                            ...
                                            <tbody>
                                                @if ($overtimerequest)
                                                @foreach ($overtimerequest as $overtimereq)
                                                <tr class="odd">
                                                   <td class="dtr-control sorting_1" tabindex="0">{{ $overtimereq->employee->username ?? 'Not updated' }}</td>
                                                   <td>
                                                    {{ $overtimereq->category->category_name ?? 'Not updated' }}
                                                   </td>
                                                   <td>{{ $overtimereq->start_time ? \Carbon\Carbon::parse($overtimereq->start_time)->format('Y M d h:i A') : 'Not updated' }}</td>
                                                   <td>{{ $overtimereq->end_time ? \Carbon\Carbon::parse($overtimereq->end_time)->format('Y M d h:i A') : 'Not updated' }}</td>

                                                   <td>
                                                    @if ($overtimereq->approval_status == 1)
                                                   ...
                                                   <td class="text-right">
                                                    ...
                                                    @if (auth()->user() && auth()->user()->role == 'HR & Administration Executive' || auth()->user()->role == 'Superadmin')
                                                       <a
                                                           class="btn btn-round btn-warning btn-icon btn-sm edit" wire:click="approverecord({{ $overtimereq->id }})"><i class="fas fa-check"></i>
                                                       </a>
                                                    @endif
                                                       ...
                                                       @if (auth()->user() && auth()->user()->role == 'HR & Administration Executive' || auth()->user()->role == 'Superadmin')
                                                           <a href="#"
                                                           class="btn btn-round btn-danger btn-icon btn-sm remove" wire:click="deleterequest({{ $overtimereq->id }})"><i
 ...
 <livewire:admin.dashboard.dep.humanresource.overtime.modal.overtimecategorymodal/>
 <livewire:admin.dashboard.dep.humanresource.overtime.modal.overtimerequestmodal/>
        </div>
    </div>

    <style>
        .rounded-border {
    border: 2px solid #b31d1d; /* Adjust the border color */
    border-radius: 10px; /* Adjust the rounding of the corners */
